$(function () {
  ...
  },
  {
    ...
    console.log('как бы хранится в DB - таблицах',arrObjects);
   ...
   /*   1. Массив скиллов (поле skills) всех людей, не должно быть повторяющихся скиллов, 
    ...
    var arrSkills = _.map(arrObjects, 'skills');
    console.log('как бы select distinct skills from ... order by 1',
    ...
    console.log('как бы select name,count(friends) from ... order by 2',
        _.map(_.sortBy(arrObjects, function (o) { return o.friends.length; }), 'name'));


    /* 3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей */
    console.log('как бы select distinct name from ... ',
        _.uniq(_.map(_.flattenDeep(_.map(arrObjects, 'friends')), 'name')));
